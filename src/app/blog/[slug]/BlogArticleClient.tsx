...
export function ClapButton({ initialClaps, slug }: ClapButtonProps) {
  ...
  const handleClap = () => {
    ...
    // Play chime reward when reaching the maximum 10 claps
  ...
  return (
    <div className="flex items-center gap-2">
      <button
        ...
        className={`group flex items-center gap-2 px-3 py-1.5 rounded-[8px] border border-border transition-all duration-200 select-none ${
          isMaxReached
            ? "opacity-60 cursor-not-allowed bg-mutedBackground"
            : "bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-900 dark:hover:bg-zinc-800 cursor-pointer"
        } ${isAnimating ? "scale-105" : "scale-100"}`}
        aria-label={
          isMaxReached
            ? "Maximum 10 claps reached"
            : `Applaud this article (${userClaps}/${MAX_USER_CLAPS})`
        }
        title={
          isMaxReached
            ? "You've given the maximum of 10 claps"
            : `Click to applaud (${MAX_USER_CLAPS - userClaps} left)`
        }
      >
        <svg
          stroke="currentColor"
      ...
      {userClaps > 0 && (
        <span className="text-[11px] font-medium text-mutedForeground select-none tabular-nums">
